...
///
/// [`context`], [`add_breadcrumb`], [`event_context`], and [`request_id`] called
/// inside `f` write to this request's own state instead of the process-wide
/// store, and concurrent scopes never see each other's values.
///
...
/// facts at boot:
///
/// - [`context`] and [`event_context`] **merge**: a notice or event reported
///   inside a scope carries the client's process-wide entries with this
///   request's own on top, and the request wins a key collision. A `version`
///   set once at startup still reaches every scoped notice.
...
/// - [`request_id`] is this request's own when it set one, and the process-wide
///   id otherwise.
///
...
/// carry it across with [`current_scope`] and [`in_scope_sync`].
#[cfg(feature = "tokio")]
pub fn sync_scope<T>(f: impl FnOnce() -> T) -> T {
    crate::scope::sync_scope(f)
}

/// Captures the current request scope, **or a fresh empty one when no scope is
/// active**. Requires the `tokio` feature.
///
/// Cheap: the returned [`ScopeHandle`](crate::ScopeHandle) shares the request's
/// state rather than copying it, so anything recorded through it reaches the
/// request that captured it. Hand it to [`in_scope`] (async) or
/// [`in_scope_sync`] (a `spawn_blocking` closure or a `std::thread::spawn` body).
///
/// Total by design: an unscoped caller gets a usable handle rather than an
/// `Option` to unwrap on a path that is often error handling already. **The cost
/// is that such a handle is a write sink.** A handle captured when no scope was
/// active — or captured inside a scope and re-entered after that scope has
/// exited — collects context and breadcrumbs into state nothing will ever read,
/// so they are silently discarded rather than reported. Note that this is a
/// change in failure mode: without a scope anywhere, an `add_breadcrumb` from a
/// `spawn_blocking` child landed in the process-wide trail, which the parent's
/// notice then read, so it *did* arrive. Capture inside the scope whose state you
/// want, and use the handle while that scope is still running.
#[cfg(feature = "tokio")]
pub fn current_scope() -> crate::scope::ScopeHandle {
    crate::scope::current_scope()
}

/// Runs `f` inside a scope captured by [`current_scope`] — the remedy for
/// [`tokio::spawn`] starting with none. Requires the `tokio` feature.
///
/// The handle shares the request's state rather than copying it, so context,
/// breadcrumbs, event context, and request id recorded inside `f` belong to the
/// request that captured the handle: they are visible both to notices reported
/// inside `f` and to later ones reported back in the original request. A scope
/// already active on this task is replaced for the duration of `f`, not merged
/// with the handle's.
///
/// See [`scope`] for the worked example.
#[cfg(feature = "tokio")]
...
/// [`in_scope`] for synchronous work — a [`tokio::task::spawn_blocking`] closure
/// or a [`std::thread::spawn`] body. Requires the `tokio` feature.
///
/// The common case, and the reason the capture API exists: synchronous database
/// drivers run on `spawn_blocking`, and "query ran" is the canonical breadcrumb.
/// Same share-not-copy semantics as [`in_scope`]; see [`scope`] for the worked
/// example.
#[cfg(feature = "tokio")]
